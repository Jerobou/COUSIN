...
$name       = $_FILES['pattern_file']['name'];
$temp_name  = $_FILES['pattern_file']['tmp_name'];
if ((isset($name)) && (!empty($name))){
       $location = './uploads/'.$folder_input_name."/";
       if(move_uploaded_file($temp_name, $location.$name)){
         $pattern_file = "./uploads/".$folder_input_name."/".$_FILES['pattern_file']['name'];
       }
   }
else {
  if ($_REQUEST['pattern_data'] != '') {
    $input_file = fopen('./uploads/'.$folder_input_name."/pattern_data.csv", 'w');
    fwrite($input_file, $_REQUEST['pattern_data']);
    fclose($input_file);
    $pattern_file = "./uploads/".$folder_input_name."/pattern_data.csv";
  }
  else {
    $pattern_file = 'nothing';
  }
...
$exec_code = "python3 ./cousin/cousin.py pattern_analysis -s ".$fasta_file." -c ".$cu_file." -g ".$genetic_code." -S ".$simulation_routine." -X ".$specific_COUSIN." -J ".$vector_analysis." -b ".$optimal_codons_file." -z ".$pattern_file." -o ./outputs/".$folder_output_name." 2>&1";
