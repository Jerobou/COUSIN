...
$sequences_format = $_REQUEST['sequences_format'];

$optimization_type = intval($_REQUEST['optimization_type']);

$number_optimizations = intval($_REQUEST['number_optimizations']);

$reverse_frequencies = intval($_REQUEST['reverse_frequencies']);
...
$exec_code = "python3 ./cousin/cousin.py optimization -s ".$fasta_file." -k ".$sequences_format." -c ".$cu_file." -g ".$genetic_code." -S ".$simulation_routine." -X ".$specific_COUSIN." -J ".$vector_analysis." -b ".$optimal_codons_file." -x ".$optimization_type." -n ".$number_optimizations." -r ".$reverse_frequencies." -o ./outputs/".$folder_output_name." 2>&1";
