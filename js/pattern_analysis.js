...
document.addEventListener('DOMContentLoaded', function() {
	...
	example_action.addEventListener("click", function(event) {
		...
		document.getElementById('pattern_data').value='';
		document.getElementById('pattern_file').value='';
	...
	if (document.getElementById('pattern_analysis-form') != null) {
		// identification du formulaire (récupération sous forme d'une variable)
		var form = document.getElementById('pattern_analysis-form');
		...
		form.addEventListener("submit", function(event) {
	        ...
	        <li> \
	          ...
	          <ul class="nav nav-pills nav-stacked collapse" id="basic_simulation_graphical_results_container" style="overflow-x:auto;"> \
					...
					<li> \
						<a href="#" class = "btn btn-default" data-target="#pattern_graphical_results_container" data-toggle="collapse" data-parent="#stacked-menu" style="font-size : large;">Display your graphical results (pattern analysis)<span class="caret arrow"></span></a> \
						<ul class="nav nav-pills nav-stacked collapse" id="pattern_graphical_results_container" style="overflow-x:auto;"> \
						</ul> \
					</li> \
					<li> \
	        ...
	        <li><a href="./php/download_results.php?folder='+folder_name+'" class = "btn btn-success" target="_blank" style="font-size : large;">Download your results</a></li> \
				...
				display_pattern_graphical_results(folder_name);
			...
			var pattern_file_check = document.getElementById("pattern_file").value;
			var pattern_data_check = document.getElementById("pattern_data").value;


			if (pattern_file_check.match(/\S/)) {
				var pattern_file_check_size = document.getElementById("pattern_file").files[0].size;
			}

			else {
				var pattern_file_check_size = 0;
			}

			if ((pattern_file_check_size >= 1073741824)) {
				accept_analysis = false
				alert('Error, the size of your "optimal codons" file is, strikingly, too big ! You should consider using the local version, reduce the size of your file, or reconsider your optimal codons file.');
			}

			else if ((pattern_file_check.match(/\S/)) && (pattern_data_check.match(/\S/))) {
				alert('Warning, we found two kind of input for the pattern data : text and file. Only the data contained in the file is kept.');
			}

			else if (!(pattern_file_check.match(/\S/)) && !(pattern_data_check.match(/\S/))) {
				alert('There is no data for the pattern analysis. A unique group containing all headers has been created.');
			}
			...
			if (accept_analysis == true) {
				...
				request.open("POST", "./php/pattern_analysis_step.php", true); // ouverture du code ajout de données php
...
function display_pattern_graphical_results(folder) {
	var request = new XMLHttpRequest();
	var new_html = "";
	...
	request.addEventListener('load', function(data) {
		...
		document.querySelector('#pattern_graphical_results_container').innerHTML = new_html;

	});

	request.open("POST", "./php/get_pattern_graphs.php",true );
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send("folder=" + folder);
