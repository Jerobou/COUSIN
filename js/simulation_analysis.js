...
document.addEventListener('DOMContentLoaded', function() {
	...
	example_action.addEventListener("click", function(event) {
		...
		document.getElementById('number_simulation').value='500';
		...
		form.addEventListener("submit", function(event) {
			...
			request.addEventListener('load', function(data){
	          ...
	          <ul class="nav nav-pills nav-stacked collapse" id="basic_simulation_graphical_results_container" style="overflow-x:auto;"> \
				...
				<li> \
				<a href="#" class = "btn btn-default" data-target="#simulation_results_container" data-toggle="collapse" data-parent="#stacked-menu" style="font-size : large;">Display your results (simulation)<span class="caret arrow"></span></a> \
				<ul class="nav nav-pills nav-stacked collapse" id="simulation_results_container" style="overflow-x:auto;"> \
				</ul> \
			</li> \
					<li> \
	          <a href="#" class = "btn btn-default" data-target="#simulation_graphical_results_container" data-toggle="collapse" data-parent="#stacked-menu" style="font-size : large;">Display your graphical results (simulation)<span class="caret arrow"></span></a> \
	          <ul class="nav nav-pills nav-stacked collapse" id="simulation_graphical_results_container" style="overflow-x:auto;"> \
	          </ul> \
	        </li> \
	        <li> \
	        ...
	        <li><a href="./php/download_results.php?folder='+folder_name+'" class = "btn btn-success" target="_blank" style="font-size : large;">Download your results</a></li> \
					...
					display_simulation_results(folder_name);
					display_basic_simulation_graphical_results(folder_name);
					display_simulation_graphical_results(folder_name);
			...
			if (accept_analysis == true) {
				...
				request.open("POST", "./php/simulation_analysis_step.php", true); // ouverture du code ajout de données php
...
function display_simulation_results(folder) {
	var request = new XMLHttpRequest();
	...
	request.addEventListener('load', function(data) {
		...
		document.querySelector('#simulation_results_container').innerHTML = new_html;


	});

	request.open("POST", "./php/get_simulation_results.php",true );
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send("folder=" + folder);
...
function display_simulation_graphical_results(folder) {
	var request = new XMLHttpRequest();
	var new_html = "";
	...
	request.addEventListener('load', function(data) {
		...
		document.querySelector('#simulation_graphical_results_container').innerHTML = new_html;

	});

	request.open("POST", "./php/get_simulation_graphs.php",true );
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send("folder=" + folder);
