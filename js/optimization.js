...
document.addEventListener('DOMContentLoaded', function() {
	...
	example_action.addEventListener("click", function(event) {
		...
		document.getElementById('sequences_format').value='DNA';
		document.getElementById('optimization_type').value='0';
		document.getElementById('number_optimizations').value='10';
		document.getElementById('reverse_frequencies').value='0';
		...
		document.getElementById('genetic_code').value='1';
		document.getElementById('sequences_format').value='DNA';
		document.getElementById('optimization_type').value='1';
		document.getElementById('number_optimizations').value='10';
	...
	if (document.getElementById('optimization-form') != null) {
		// identification du formulaire (récupération sous forme d'une variable)
		var form = document.getElementById('optimization-form');
		...
		form.addEventListener("submit", function(event) {
			...
			request.addEventListener('load', function(data){
				...
				if (simulation_routine_checked == true) {
					...
					<li> \
						<a href="#" class = "btn btn-default" data-target="#optimization_container" data-toggle="collapse" data-parent="#stacked-menu" style="font-size : large;">Display your optimized sequences<span class="caret arrow"></span></a> \
						<ul class="nav nav-pills nav-stacked collapse" id="optimization_container" style="overflow-x:auto;"> \
						</ul> \
					</li> \
					<li> \
			...
			if (accept_analysis == true) {
				...
				request.open("POST", "./php/optimization_step.php", true); // ouverture du code ajout de données php
...
function display_sequences(folder) {
	var request = new XMLHttpRequest();
	...
	request.addEventListener('load', function(data) {
		...
		if (table_line.length < 150) {
			for (var i=0; i < table_line.length; i++) {
				new_html += build_sequences_html(table_line[i]);
			}
		}
		...
		document.querySelector('#optimization_container').innerHTML = new_html;

	});

	request.open("POST", "./php/get_optimized_sequences.php",true );
	request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	request.send("folder=" + folder);

}

function build_sequences_html(line) {
	if (line[0] == '>') {
		return ("<p style= 'white-space: pre-wrap; font-family: monospace; font-size: 11px;'><b>" + line + "</b></p>");
		...
		line = chunk(line, 80).join("<br>");
		return("<p style= 'white-space: pre-wrap; font-family: monospace; font-size: 11px;'>" + line + '</p><br>');
